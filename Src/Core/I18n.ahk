...
class I18n {
    ...
    static _Build() {
        ...
        s["Prefs.FileDefaultLimit"]     := ["Files in normal results", "普通结果里的文件数"]
        s["Prefs.FileMaxResults"]       := ["Max results", "最多结果数"]
        s["Prefs.UseEverything"]       := ["Use Everything when it is running (whole disk)", "Everything 运行时用它搜索 (全盘)"]
        s["Prefs.EverythingFilter"]     := ["Filter", "过滤条件"]
        ...
        s["Prefs.ScopeFolders"]         := ["Folders", "文件夹"]
        ...
        s["Prefs.Section.Theme"]          := ["Theme", "主题"]
        s["Prefs.Section.Size"]           := ["Size", "大小"]
        s["Prefs.Section.FeatureOptions"] := ["Feature options", "功能选项"]
        s["Prefs.Section.AppIndex"]       := ["Index", "索引范围"]
        s["Prefs.Section.AppResults"]     := ["In search results", "搜索结果"]
        s["Prefs.Section.FileStart"]      := ["Starting a file search", "开始文件搜索"]
        s["Prefs.Section.FileResults"]    := ["Results", "搜索结果"]
        s["Prefs.Section.Everything"]     := ["Everything (whole-disk search)", "Everything (全盘搜索)"]
        s["Prefs.Section.BuiltinIndex"]   := ["Built-in index (when Everything is not running)", "内置索引 (Everything 没有运行时)"]
        s["Prefs.Section.Options"]        := ["Options", "选项"]
        s["Prefs.TypeShort.File"]         := ["File", "文件"]
        s["Prefs.TypeShort.Folder"]       := ["Folder", "文件夹"]
        s["Prefs.TypeShort.Command"]      := ["Command", "命令行"]
        s["Prefs.TypeShort.Url"]          := ["URL", "网址"]
        ; General
        ...
        s["Prefs.Width.Desc"]           := ["The width grows with the Windows display scale. More results scroll with ↑ ↓; the window grows with the results.", "宽度按 Windows 的缩放比例放大; 更多结果用 ↑ ↓ 滚动, 窗口高度随结果数量变化"]
        ...
        s["Prefs.TerminalPrefix.Desc"]  := ["Type >command to run it in the shell, e.g. >ipconfig /all. 'Open terminal here' uses the same shell.", "输入 >命令 在终端里运行, 例如 >ipconfig /all; '在此处打开终端' 也用这个程序"]
        s["Prefs.TerminalShell"]        := ["Shell", "终端程序"]
        s["Prefs.FileKeywords"]         := ["File keywords", "文件关键字"]
        s["Prefs.FileKeywords.Desc"]    := ["e.g. open report searches only files, folder bk only folders. Separate with commas.", "例如 open report 只搜文件, folder bk 只搜文件夹; 用逗号分隔"]
        s["Prefs.FolderKeywords"]       := ["Folder keywords", "文件夹关键字"]
        ...
        s["Prefs.EverythingPath"]       := ["Location", "安装位置"]
        ...
        ; Applications
        s["Prefs.AppFolders"]           := ["Folders", "文件夹"]
        ...
        s["Prefs.AppDepth.Desc"]        := ["Subfolder levels to scan. The index also refreshes itself in the background at this interval.", "往下扫描几层子文件夹; 索引还会按这个间隔在后台自动更新"]
        ...
        s["Prefs.RefreshMinutes"]       := ["Refresh every (min)", "刷新间隔 (分钟)"]
        ...
        s["Prefs.Fallbacks"]            := ["Fallback searches", "兜底搜索"]
        s["Prefs.Fallbacks.Desc"]       := ["Shown when nothing matches: engine Ids (google, bing, baidu...) or files. Separate with commas.", "没有任何结果时显示: 引擎的 Id (google、bing、baidu...) 或 files (文件搜索); 用逗号分隔"]
        ...
        s["Prefs.QSExplorer"]           := ["Explorer hotkey", "资源管理器热键"]
        s["Prefs.QSTotalCmd"]           := ["Total Commander hotkey", "Total Commander 热键"]
