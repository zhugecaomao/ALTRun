...
## 升级
### 一键更新
ALTRun 每次启动时到 GitHub 检查一次新版本 (偏好设置 → 通用 → "启动时检查更新"; 也可以在托盘菜单里点 "检查更新")。发现新版本时:
- **立即更新**: 下载新版本, 核对 SHA256 校验值, 替换程序文件后自动重新启动。`ALTRun.json`、`Data\`、`Themes\` 不会改动
- **查看更新内容**: 打开 GitHub 上这个版本的说明
- **以后再说**: 下次启动再提示

更新过程中出现任何问题 (网络、校验、写入权限), 原来的程序保持不变, 并提示打开下载页面手动更新。命令行 `ALTRun.exe -Update` 会直接检查并安装更新, 不询问。

以下情况不能一键更新, 仍然提示打开下载页面或给出命令: 用 Scoop / winget 安装 (用它们的升级命令)、运行源码 `ALTRun.ahk`、程序放在没有写入权限的文件夹里。
