...
- **应用搜索**: 自动索引开始菜单、桌面和应用商店应用, 中文名称支持拼音首字母 ("wx" → 微信); 不需要的应用按 `Ctrl+Del` 从结果中删除
...
| `Ctrl+Del` (光标在末尾) | 删除选中项 (自定义命令 / 片段 / 搜索引擎 / 剪贴板历史), 删除前确认; 应用: 从搜索结果中删除 (不卸载, 可在偏好设置 → 应用搜索 里恢复) |
...
| 应用搜索 | 索引的文件夹、文件类型、排除规则、已删除的应用 (删掉一行即可恢复)、拼音首字母、应用商店应用, 重建索引 |
