...
; 不依赖任何外部 API/网络请求。
;
; Lib/Kanji.txt: 一整行, 空格分隔的"简体+繁体"两字词, 例如 "锕錒 皑皚 嗳噯"
...
Class Kanji {
    ...
    static _Load() {
        ...
        file := A_ScriptDir "\Lib\Kanji.txt"
        if !FileExist(file) {
            g_LOG.Debug("Kanji: Lib\Kanji.txt not found, simplified/traditional conversion disabled")
            return
        }
